...
func TestDummy(t *testing.T) {
	...
	_, err := assetvfs.Stat("notfound.txt")
	t.Logf("error = %#v", err)

	perr := err.(*os.PathError)
	t.Logf("error.Err = %#v", perr.Err)
	t.Logf("error.Err = %T", perr.Err)
	t.Logf("error.Err = %#v", perr.Err.Error())

	/*
		assetvfs := vfs.OS("./example/assets")
		_, err := assetvfs.ReadDir("notfound")
		t.Logf("error = %#v", err)
	*/
	/*
		perr := err.(*os.PathError)
...
func TestFileSystem(t *testing.T) {
	var vfsFS vfs.FileSystem = bindatafs.New(nil, nil, nil)
